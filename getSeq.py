...
#Take string, and split into array of lines 70 chars each using textwrap
def print70(strToWrap):
	lineWidth = 70
	strArr = textwrap.wrap(strToWrap,lineWidth)
	for i in strArr:
		print i
	...
	if i is not 0:
		...
		if not rev:
			...
			while (index <= stop):
				cutSeq += seq[index]
				index += 1
			print70(cutSeq) #cutSeq is the gene cut from the entire seq
		elif rev:
